...
public class GalleryActivity extends Activity {
	...
	public void flipFavIcon(View view, boolean enable) {
		ImageView iv = (ImageView) view.findViewById(R.id.imageViewFav);
		if (enable) {
			iv.setImageDrawable(getResources().getDrawable(R.drawable.favorites_yellow));
		} else {
			iv.setImageDrawable(getResources().getDrawable(R.drawable.favorites_grey));
		}
	}
	...
	private class FetchImagesTask extends AsyncTask<String, Void, List<Photo>> {
		...
		protected void onPostExecute(final List<Photo> result) {
			...
			final ArrayAdapter<Photo> adapter = new PhotoListAdapter(GalleryActivity.this,
					R.layout.listview_photo_item, result);
			((PhotoListAdapter) adapter).setDbManager(dbManager);
			...
			listView.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
				...
				public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
					...
					boolean flag = dbManager.getPhotoDAO().updateFavIcon(photo);
					flipFavIcon(view, flag);
					return true;
				}
			});
